...
define([], function () {
    ...
    function initAiDetailButtons(root) {
        ...
        const labels = {
            sessions: root.dataset.labelSessions || 'Sesiones',
            messages: root.dataset.labelMessages || 'Mensajes',
            time: root.dataset.labelTime || 'Tiempo',
            last30d: root.dataset.labelLast30d || 'Últimas 30 días:',
            date: root.dataset.labelDate || 'Fecha',
            level: root.dataset.labelLevel || 'Nivel',
            noRecent: root.dataset.labelNoRecent || 'Sin sesiones en los últimos 30 días.',
            errorLoading: root.dataset.labelErrorLoading || 'Error al cargar los datos.',
        };
        if (!detailUrl) return;
        ...
        root.addEventListener('click', function (e) {
            ...
            .then(function (data) {
                modalBody.innerHTML = data.error
                    ...
                    : renderAiDetail(data, labels);
            })
            .catch(function () {
                modalBody.innerHTML = '<p class="text-danger">' + labels.errorLoading + '</p>';
            });
        });
    ...
    function renderAiDetail(data, labels) {
        const levelColors = { 1: '#C8102E', 2: '#e07b00', 3: '#0D1520' };
        let html = '<div class="d-flex mb-3 text-center">' +
            stat(data.total_sessions, labels.sessions) +
            stat(data.total_messages, labels.messages) +
            stat(data.total_minutes + ' min', labels.time) +
            '</div>';

        if (!data.sessions || !data.sessions.length) {
            return html + '<p class="text-muted small">' + labels.noRecent + '</p>';
        }

        html += '<p class="text-muted small mb-1">' + labels.last30d + '</p>' +
            '<table class="table table-sm table-borderless mb-0"><thead><tr>' +
            '<th class="small">' + labels.date + '</th><th class="small">' + labels.level + '</th>' +
    ...
    function initAdvisorButtons(root) {
        ...
        const labels     = {
            typing: root.dataset.labelTyping || 'Escribiendo…',
            connectionError: root.dataset.labelConnectionError || 'Error de conexión. Inténtalo de nuevo.',
        };
        ...
        sendBtn.addEventListener('click', function () {
            sendAdvisorMessage(advisorUrl, sesskey, lang, msgBox, input, sendBtn, labels);
        });
    ...
    function sendAdvisorMessage(advisorUrl, sesskey, lang, msgBox, input, sendBtn, labels) {
        ...
        const typingId = appendTyping(msgBox, labels.typing);
        ...
        .catch(function () {
            removeTyping(msgBox, typingId);
            appendMessage(msgBox, 'error', labels.connectionError);
        })
        .finally(function () {
    ...
    function appendTyping(msgBox, typingLabel) {
        const id = 'pharos-typing-' + Date.now();
        const div = document.createElement('div');
        div.id = id;
        div.className = 'pharos-advisor-msg pharos-advisor-msg--typing';
        div.setAttribute('aria-label', typingLabel || 'Escribiendo…');
    ...
    function initMotivateButtons(root) {
        ...
        const sesskey     = root.dataset.sesskey;
        const lang        = root.dataset.advisorLang || 'es';
        const connectionError = root.dataset.labelConnectionError || 'Error de conexión. Inténtalo de nuevo.';
